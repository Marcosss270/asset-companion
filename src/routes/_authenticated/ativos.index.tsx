  ...
  const { data: ativos = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("ativos")
        .select("id, codigo_unico, nome, marca, modelo, numero_serie, status, localizacao, responsavel, categoria_id, categorias(nome)")
  ...
  return (
    <div className="max-w-[1400px] mx-auto">
      ...
      <div className="bg-card border border-border rounded-xl p-4 mb-6 shadow-card flex flex-wrap gap-3 items-center">
        ...
        <select
          value={statusFilter}
          onChange={(e) => setStatusFilter(e.target.value as AtivoStatus | "todos")}
          className="px-3 py-2 bg-secondary border-none rounded-lg text-sm outline-none cursor-pointer"
        >
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-border">
              {isLoading ? (
                ...
                <tr><td colSpan={6} className="px-6 py-12 text-center text-muted-foreground text-sm">Nenhum ativo encontrado.</td></tr>
              ) : (
                filtered.map((a) => (
